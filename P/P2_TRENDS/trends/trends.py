...
def extract_words(text):
 ...
 #   if word !='':
 #       words.append(word)
 #   return words  # Replace this line
    for letter in text:
...
def analyze_tweet_sentiment(tweet):
    ...
    for word in words:
        ...
        if has_sentiment(s):
            average += sentiment_value(s)
            i += 1
    if i == 0:
        return make_sentiment(None)
    else:
        return make_sentiment(average / i)
...
def find_centroid(polygon):
    ...
    for i in range(len(polygon)-1):
        ce_lat += (( latitude( polygon[i] ) + latitude( polygon[i+1] ))
                * ((latitude( polygon[i]) * longitude( polygon[i+1]) ) 
                - (latitude( polygon[i+1]) * longitude( polygon[i]) ))) 
        ce_lon += (( longitude( polygon[i] ) + longitude( polygon[i+1] ))
                * ((latitude( polygon[i]) * longitude( polygon[i+1]) ) 
                - (latitude( polygon[i+1]) * longitude( polygon[i]) ))) 
        area += ((latitude( polygon[i] ) * longitude( polygon[i+1] )) 
              - (longitude( polygon[i] ) * latitude( polygon[i+1] )))
    area /= 2
    if area != 0:
        ce_lat /= 6 * area
        ce_lon /= 6 * area
    else:
        ce_lat = latitude( polygon[0] )
        ce_lon = longitude( polygon[0] )
    area = abs(area)
    return (ce_lat, ce_lon, area) 
...
def find_state_center(polygons):
    ...
    for polygon in polygons:
        ce_lat, ce_lon, area = find_centroid(polygon)
        ce_lat_state += ce_lat * area
        ce_lon_state += ce_lon * area
        area_state += area
    ce_lat_state /= area_state
    ce_lon_state /= area_state
    return make_position(ce_lat_state, ce_lon_state)
...
def group_tweets_by_state(tweets):
    ...
    tweets_by_state = {}
    "*** YOUR CODE HERE ***"
    us_states_centroids = centroids_for_objects(us_states)
    for tweet in tweets:
        position_tweet = tweet_location(tweet)
        closest_state = min_distance(position_tweet, us_states_centroids)
...
def centroids_for_objects(objects):
    """Gets a dictionary with keys and polygons coordinates and gives 
    a new dictionary with the same keys and their centroids"""
    centroids = {}
    for item in objects:
        centroids[item] = find_state_center(objects[item])
    return centroids 
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        tweets_sentiment_value = 0
        state_sentiment = False
        i = 0
        for tweet in tweets_by_state[state]: 
            s = analyze_tweet_sentiment(tweet)
            if has_sentiment(s):
                tweets_sentiment_value += sentiment_value(s)
                i += 1
                state_sentiment = True
        if state_sentiment:
            tweets_sentiment_value /= i
            averaged_state_sentiments[state] = tweets_sentiment_value
