...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < goal and opponent_score < goal:
        ...
        # Determine which dice is to be used
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    max_score = 0
    for i in range(1, 11):
        avg = make_averaged(roll_dice, 100000)(i, dice)
        print('{} dice scores {} on average'.format(i, avg))
        if max_score < avg:
            max_score = avg
...
def bacon_strategy(score, opponent_score):
    ...
    if (bacon_points >= BACON_MARGIN):
        return 0
    return BASELINE_NUM_ROLLS # Statement Replaced
...
def swap_strategy(score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    bacon_points = 1 + max(opponent_score // 10, opponent_score % 10)
    if (((bacon_points >= BACON_MARGIN) and ((score + bacon_points)/2 != opponent_score)) or 
       ((score + bacon_points)*2 == opponent_score)):
        return 0 # Statement Replaced
    return BASELINE_NUM_ROLLS # Statement Replaced
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    four_sided_dice = False
    if (score + opponent_score) % 7 == 0:
        b_margin = 5
        four_sided_dice = True
    if ((((bacon_points >= b_margin) or 
       (score + bacon_points + opponent_score) % 7 == 0) and 
       ((score + bacon_points)/2 != opponent_score)) or 
       ((score + bacon_points)*2 == opponent_score)):
        return 0 # Statement Replaced
    if four_sided_dice:
        rolls -= 2
    if (opponent_score > 50) and (score < 1.2 * opponent_score):
        rolls += 1
    elif (score > 80) and (score > 1.1 * opponent_score):
        rolls -= 1
    return rolls
